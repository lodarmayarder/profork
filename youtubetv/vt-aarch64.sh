...
APPLINK=$(curl -fsSL https://api.github.com/repos/shy1132/VacuumTube/releases/latest 2>/dev/null \
  | jq -r '.assets[] | select(.name | endswith(".AppImage") and (.name | test("(?i)(arm64)"))) | .browser_download_url' 2>/dev/null \
  | head -n1)
APPLINK=${APPLINK:-https://github.com/shy1132/VacuumTube/releases/latest/download/VacuumTube-arm64.AppImage}
...
echo 'LD_LIBRARY_PATH="/userdata/system/pro/.dep:${LD_LIBRARY_PATH}" DISPLAY=:0.0 /userdata/system/pro/'$appname'/'$appname'.AppImage  --appimage-extract-and-run --no-sandbox --test-type "$@"' >> $launcher
######################################################################
######################################################################
######################################################################
######################################################################

#!/bin/bash
# Free ROM installer for Batocera
# - Installs REG-Linux freebies (direct raw URLs)
# - Installs EmuDeck homebrew by downloading the repo zip once and copying .zip ROMs into /userdata/roms/<system>
#
...
# -----------------------------
# REG-Linux freebies ONLY (no Batocera downloads)
# -----------------------------
...
if [[ "$EMUDECK" == "1" ]]; then
  ...
  IGNORE_PATTERNS=(
    ...
    "rem"            # REM.zip (broad; fine for this set)
    "petrophobia"    # Petrophobia.zip
    "anguna"         # any Anguna zip if present (duplicate vs REG)
    "wingwarriors"   # WingWarriors.zip / Wing Warriors.zip

...
APPLINK=$(curl -s https://api.github.com/repos/ivan-hc/Steam-appimage/releases/latest \
  | jq -r '.assets[] | select(.name | endswith(".AppImage")) | .browser_download_url')
ORIGIN="github.com/ivan-hc/Steam-appimage" # credit & info
# --------------------------------------------------------------------
# --------------------------------------------------------------------
# --------------------------------------------------------------------
# --------------------------------------------------------------------
...
echo 'LD_LIBRARY_PATH="/userdata/system/pro/.dep:${LD_LIBRARY_PATH}" DISPLAY=:0.0 RIM_SYS_NVLIBS=1 RIM_ALLOW_ROOT=1 /userdata/system/pro/'$appname'/'$appname'.AppImage -gamepadui "$@"' >> $launcher
######################################################################
######################################################################
######################################################################
######################################################################
...
dos2unix $launcher
chmod a+x $launcher
cp $launcher /userdata/roms/ports/Steam-AppImage.sh 2>/dev/null

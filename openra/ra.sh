...
APPLINK=APPLINK=$(curl -s https://api.github.com/repos/OpenRA/OpenRA/releases/latest | jq -r '.assets[] | select(.name | test("Red-Alert.*x86_64\\.AppImage$")) | .browser_download_url')
...
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O $pro/$appname/extra/icon.png https://github.com/profork/profork/raw/master/openra/extra/cncra.png; chmod a+x $dep/* 2>/dev/null; cd ~/
...
wget -q -O $icon https://github.com/profork/profork/raw/master/openra/extra/cncra.png
# //

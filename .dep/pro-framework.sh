...
#echo -e "${A}██${X}  ${A}downloading additional files"
	xurl=https://github.com/profork/profork/raw/master/$app/extras
...
# include display output: 
   ...
   mkdir -p /userdata/system/pro/.dep 2>/dev/null
      if [[ ( -e "$tput" && "$(wc -c "$tput" | awk '{print $1}')" < "444" ) || ( ! -e "$tput" ) ]]; then
         rm "$tput" 2>/dev/null
         wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O "$tput" https://github.com/profork/profork/raw/master/dep/.tput
      fi
      if [[ ( -e "$libtinfo" && "$(wc -c "$libtinfo" | awk '{print $1}')" < "444" ) || ( ! -e "$libtinfo" ) ]]; then
         rm "$libtinfo" 2>/dev/null
         wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O $libtinfo https://github.com/profork/profork/raw/master/.dep/.libtinfo.so.6
      fi
...
# include display output: 
#
url_tput=https://github.com/profork/profork/raw/master/.dep/tput
url_libtinfo=https://github.com/profork/profork/raw/master/.dep/libtinfo.so.6
	#
   tput=/userdata/system/pro/.dep/tput
   libtinfo=/userdata/system/pro/.dep/libtinfo.so.6
   mkdir -p /userdata/system/pro/.dep 2>/dev/null
...
cd /tmp/ 
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O "/tmp/batocera.pro-loader.mp4" "https://github.com/profork/profork/raw/master/.dep/loader.mp4"
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O "/tmp/batocera.pro-$app.sh" "https://github.com/profork/profork/raw/master/$app/$app.sh"
